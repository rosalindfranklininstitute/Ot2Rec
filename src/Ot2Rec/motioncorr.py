...
class Motioncorr:
    ...
    def _get_command(self, image, extra_info=None):
        ...
        cmd = [
            ...
            gpu_number,
            "-GpuMemUsage",
            str(self.params["System"]["gpu_memory_usage"]),
            "-Gain",
            self.params["MC2"]["gain_reference"],
            "-Tol",
            str(self.params["MC2"]["tolerance"]),
            "-Patch",
            ",".join(str(i) for i in self.params["MC2"]["patch_size"]),
            "-Iter",
            str(self.params["MC2"]["max_iterations"]),
            "-Group",
            "1" if self.params["MC2"]["use_subgroups"] else "0",
            "-FtBin",
            str(ftbin),
            "-PixSize",
            str(self.params["MC2"]["pixel_size"]),
            "-Throw",
            str(self.params["MC2"]["discard_frames_top"]),
            "-Trunc",
            str(self.params["MC2"]["discard_frames_bottom"]),
            "-LogFile",
            out_path + ".log",
        ]
    ...
    def update_mc2_metadata(self):
        ...
        _to_append = self.meta.loc[
            self.meta["output"].apply(lambda x: os.path.isfile(x))
        ]
        self.meta_out = pd.concat([self.meta_out, _to_append], ignore_index=True)
        self.meta = self.meta.loc[
            ~self.meta["output"].apply(lambda x: os.path.isfile(x))
        ]
...
def update_yaml(args):
    ...
    if is_old_project:
        merged_md = master_md.merge(mc2_md, how="outer", indicator=True)
...
def run(exclusive=True, args_in=None):
    ...
    master_md = mdMod.read_md_yaml(
        project_name=project_name, job_type="motioncorr", filename=master_md_file
    )
