...
class Recon:
    ...
    def _get_internal_metadata(self):
        ...
        self._recon_images = pd.DataFrame(
            columns=["ts", "align_output", "recon_output"]
        )
        for curr_ts in self.params["System"]["process_list"]:
            subfolder = f"{self.basis_folder}/{self.rootname}_{curr_ts}{self.suffix}"
            _to_append = pd.DataFrame(
                {
                    "ts": [curr_ts],
                    "align_output": [
                        f"{subfolder}/{self.rootname}_{curr_ts}{self.suffix}_ali.mrc"
                    ],
                    "recon_output": [
                        f"{subfolder}/{self.rootname}_{curr_ts}{self.suffix}_rec.mrc"
                    ],
                }
            )
            self._recon_images = pd.concat(
                [self._recon_images, _to_append],
                ignore_index=True,
            )
    ...
    def _check_reconned_images(self):
        ...
        if len(self.meta_out) > 0:
            self._missing = self.meta_out.loc[
                ~self.meta_out["recon_output"].apply(lambda x: os.path.isfile(x))
            ]
        ...
        _ignored = self._recon_images[
            self._recon_images.recon_output.isin(self.meta_out.recon_output)
        ]
        if len(_ignored) > 0 and len(_ignored) < len(self._recon_images):
            ...
            self.logObj(
                f"Info: All specified images had been processed. Nothing will be done."
            )
        ...
        self._process_list = (
            self._recon_images["ts"].sort_values(ascending=True).unique().tolist()
        )
...
runtime.Trimvol.any.reorient = <trimvol_reorient>
        ...
        convert_dict = {
            ...
            "do_pos": 1
            if self.params["BatchRunTomo"]["positioning"]["do_positioning"]
            else 0,
            "pos_thickness": self.params["BatchRunTomo"]["positioning"][
                "unbinned_thickness"
            ],
            "corr_ctf": 1
            if self.params["BatchRunTomo"]["aligned_stack"]["correct_ctf"]
            else 0,
            "erase_gold": 1
            if self.params["BatchRunTomo"]["aligned_stack"]["erase_gold"]
            else 0,
            "filter_stack": 1
            if self.params["BatchRunTomo"]["aligned_stack"]["2d_filtering"]
            else 0,
            "stack_bin_factor": self.params["BatchRunTomo"]["aligned_stack"][
                "bin_factor"
            ],
            "recon_thickness": self.params["BatchRunTomo"]["reconstruction"][
                "thickness"
            ],
            "use_sirt": 1
            if self.params["BatchRunTomo"]["reconstruction"]["use_sirt"]
            else 0,
            "sirt_iter": self.params["BatchRunTomo"]["reconstruction"]["sirt_iter"],
            "run_trimvol": 1
            if self.params["BatchRunTomo"]["postprocessing"]["run_trimvol"]
            else 0,
    ...
    def _get_brt_recon_command(
        ...
        cmd = [
            ...
            "./recon.adoc",
            "-RootName",
            f"{self.rootname}_{curr_ts}",
            "-CurrentLocation",
            self._path_dict[curr_ts],
            "-StartingStep",
            "8" if not ext else "0",
            "-EndingStep",
            "20" if not ext else "0",
        ]
        ...
        for curr_ts in tqdm_iter:
            ...
            if ext:
                batchruntomo = subprocess.run(
                    self._get_brt_recon_command(curr_ts, ext=True),
            ...
            batchruntomo = subprocess.run(
                self._get_brt_recon_command(curr_ts, ext=False),
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                encoding="ascii",
                check=True,
    ...
    def update_recon_metadata(self):
        ...
        _to_append = self._recon_images.loc[
            self._recon_images["recon_output"].apply(lambda x: os.path.isfile(x))
        ]
        self.meta_out = pd.concat([self.meta_out, _to_append], ignore_index=True)
        self._recon_images = self._recon_images.loc[
            ~self._recon_images["recon_output"].apply(lambda x: os.path.isfile(x))
        ]
...
def update_yaml(args):
    ...
    if not os.path.isfile(recon_yaml_name):
        logger(level="error", message="IMOD reconstruction config file not found.")
        raise IOError(
            "Error in Ot2Rec.main.update_recon_yaml: reconstruction config file not found."
        )
    if not os.path.isfile(align_yaml_name):
        logger(level="error", message="IMOD alignment config file not found.")
        raise IOError(
            "Error in Ot2Rec.main.update_recon_yaml: alignment config file not found."
        )
    ...
    if is_old_project:
        merged_md = align_md.merge(recon_md, how="outer", indicator=True)
    ...
    recon_params = prmMod.read_yaml(
        project_name=args.project_name, filename=recon_yaml_name
    )
    align_params = prmMod.read_yaml(
        project_name=args.project_name, filename=align_yaml_name
    )

    recon_params.params["System"]["output_rootname"] = align_params.params["System"][
        "output_rootname"
    ]
    recon_params.params["System"]["output_suffix"] = align_params.params["System"][
        "output_suffix"
    ]
    recon_params.params["System"]["process_list"] = unique_ts_numbers

    recon_params.params["BatchRunTomo"]["setup"] = {
        key: value
        for key, value in align_params.params["BatchRunTomo"]["setup"].items()
...
def run(exclusive=True, args_in=None):
    ...
    align_md = mdMod.read_md_yaml(
        project_name=project_name, job_type="reconstruct", filename=align_md_file
    )
