...
class Align:
    ...
    def _get_internal_metadata(self):
        ...
        self._align_images = pd.DataFrame(
            ...
            _to_append = pd.DataFrame(
                {
                    "ts": [curr_ts],
                    "stack_output": [
                        f"{self.basis_folder}/{subfolder_name}/{subfolder_name}.{self.output_ext}"
                    ],
                    "align_output": [
                        f"{self.basis_folder}/{subfolder_name}/{subfolder_name}_ali.mrc"
                    ],
                },
            )
            self._align_images = pd.concat(
                [self._align_images, _to_append],
                ignore_index=True,
            )
    ...
    def _check_aligned_images(self):
        ...
        # Read in serialised metadata and turn into DataFrame if record exists
            ...
            _meta_record = mdMod.read_md_yaml(
        ...
        if len(self.meta_out) > 0:
            self._missing = self.meta_out.loc[
                ~self.meta_out["align_output"].apply(lambda x: os.path.isfile(x))
            ]
            self._missing_specified = pd.DataFrame(columns=self.meta.columns)

            for curr_ts in self.params["System"]["process_list"]:
                _to_append = self._missing[self._missing["ts"] == curr_ts]
                self._missing_specified = pd.concat(
                    [self._missing_specified, _to_append],
                    ignore_index=True,
                )
            self._merged = self.meta_out.merge(
                self._missing_specified, how="left", indicator=True
            )
            self.meta_out = self.meta_out[self._merged["_merge"] == "left_only"]

            if len(self._missing_specified) > 0:
                self.logObj(
                    f"Info: {len(self._missing_specified)} images in record missing in folder. "
                    "Will be added back for processing."
                )

        # Drop the items in input metadata if they are in the output record
        _ignored = self._align_images[
            self._align_images.align_output.isin(self.meta_out.align_output)
        ]
        if len(_ignored) > 0 and len(_ignored) < len(self._align_images):
            self.logObj(
                f"Info: {len(_ignored)} images had been processed and will be omitted."
            )
        elif len(_ignored) == len(self._align_images):
            self.logObj(
                "Info: All specified images had been processed. Nothing will be done."
            )
    ...
    def _sort_tilt_angles(self, curr_ts: int):
        """
        ...
        # Sorting
        meta_ts_sorted = meta_ts.sort_values(
            by="angles",
            axis=0,
            ascending=True,
        )
...
comparam.align.tiltalign.WeightWholeTracks = <weight_contours>
        ...
        convert_dict = {
            ...
            "use_rawtlt": 1
            if self.params["BatchRunTomo"]["setup"]["use_rawtlt"]
            else 0,
            "pixel_size": self.params["BatchRunTomo"]["setup"]["pixel_size"]
            * self.params["BatchRunTomo"]["setup"]["stack_bin_factor"],
            "rot_angle": self.params["BatchRunTomo"]["setup"]["rot_angle"],
            "excl_views": ""
            if self.params["BatchRunTomo"]["setup"]["excluded_views"] == [0]
            ...
            "stack_bin_factor": self.params["BatchRunTomo"]["setup"][
                "stack_bin_factor"
            ],
            "aligned_bin_factor": self.params["BatchRunTomo"]["setup"][
                "aligned_bin_factor"
            ],
            "delete_old_files": 1
            if self.params["BatchRunTomo"]["preprocessing"]["delete_old_files"]
            else 0,
            "remove_xrays": 1
            if self.params["BatchRunTomo"]["preprocessing"]["remove_xrays"]
            else 0,
            ...
            "num_iterations": self.params["BatchRunTomo"]["patch_track"][
                "num_iterations"
            ],
            "adj_tilt_angles": 1
            if self.params["BatchRunTomo"]["patch_track"]["adjust_tilt_angles"]
            else 0,
            "num_surfaces": self.params["BatchRunTomo"]["fine_align"]["num_surfaces"],
            "mag_option": {"all": 1, "group": 3, "fixed": 0}[
                self.params["BatchRunTomo"]["fine_align"]["mag_option"]
            ],
            "tilt_option": {"all": 1, "group": 5, "fixed": 0}[
                self.params["BatchRunTomo"]["fine_align"]["tilt_option"]
            ],
            ...
            "use_robust": 1
            if self.params["BatchRunTomo"]["fine_align"]["use_robust_fitting"]
            else 0,
            "weight_contours": 1
            if self.params["BatchRunTomo"]["fine_align"]["weight_all_contours"]
            else 0,
        }
    ...
    def _get_brt_align_command(self, curr_ts: int):
        """
        ...
        cmd = [
            ...
            "./align.adoc",
            "-RootName",
            self.params["System"]["output_rootname"] + f"_{curr_ts}",
            "-CurrentLocation",
            f"{self.basis_folder}/{self.rootname}_{curr_ts}{self.suffix}",
            "-StartingStep",
            "0",
    ...
    def align_stack(self, ext=False):
        ...
        for curr_ts in tqdm_iter:
            ...
            batchruntomo = subprocess.run(
                self._get_brt_align_command(curr_ts),
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                encoding="ascii",
                check=True,
            ...
            try:
                ...
                error_count += 1
                self.logObj(
                    f"Batchruntomo: An error has occurred ({batchruntomo.returncode}) "
                    f"on stack{curr_ts}."
                )
        ...
        if error_count == 0:
            ...
            self.logObj(
                "WARNING: All Ot2Rec-align (IMOD) jobs finished."
    ...
    def update_align_metadata(self, ext=False):
        ...
        _to_append = self._align_images.loc[
            self._align_images["align_output"].apply(lambda x: os.path.isfile(x))
        ]
        self.meta_out = pd.concat([self.meta_out, _to_append], ignore_index=True)
        self._align_images = self._align_images.loc[
            ~self._align_images["align_output"].apply(lambda x: os.path.isfile(x))
        ]
...
def update_yaml(args, logger=None):
    ...
    if is_old_project:
        merged_md = mc2_md.merge(align_md, how="outer", indicator=True)
        unprocessed_images = merged_md.loc[lambda x: x["_merge"] == "left_only"]
    else:
        unprocessed_images = mc2_md

    unique_ts_numbers = (
        unprocessed_images["ts"].sort_values(ascending=True).unique().tolist()
    )

    # Read in ctffind yaml file, modify, and update
    # read in MC2 yaml as well (some parameters depend on MC2 settings)
    align_params = prmMod.read_yaml(
        project_name=args.project_name, filename=align_yaml_name
    )
...
def update_yaml_stacked(args):
    ...
    rootname = (
        ...
        if args.file_prefix.value != ""
        else args.project_name.value
    )
    ...
    ts_list = [
        int(i.split("/")[-1].replace(f"{rootname}_", "").replace(f"{suffix}.st", ""))
        for i in st_file_list
    ]

    # Read in and update YAML parameters
    align_yaml_name = project_name + "_align.yaml"
    align_params = prmMod.read_yaml(project_name=project_name, filename=align_yaml_name)
...
def run(
    ...
    if exclusive:
        ...
        parser.add_argument("project_name", type=str, help="Name of current project")
    ...
    align_config = prmMod.read_yaml(project_name=project_name, filename=align_yaml)
    if not ext:
        mc2_md = mdMod.read_md_yaml(
            project_name=project_name, job_type="align", filename=mc2_md_file
        )
...
def get_align_stats(exclusive=True, args_in=None):
    ...
    if not os.path.isfile(align_md_name):
        raise IOError(
            "Error in Ot2Rec.main.get_align_stats: alignment metadata file not found."
        )
    ...
    with open(align_md_name, "r") as f:
        aligned_ts = pd.DataFrame(yaml.load(f, Loader=yaml.FullLoader))[
            "ts"
        ].values.tolist()
    ...
    for curr_ts in aligned_ts:
        ...
        if not os.path.isfile(target_file_path):
            raise IOError(
                "Error in Ot2Rec.main.get_align_stats: alignment log file (taLocals) not found."
            )
    ...
    with open(f"{rootname}_imod_align_stats.yaml", "w") as f:
        yaml.dump(
            stats_df.reset_index().to_dict(orient="records"),
            f,
            sort_keys=False,
            indent=4,
        )

    stats_df.sort_values(by="Error mean (nm)", inplace=True)
