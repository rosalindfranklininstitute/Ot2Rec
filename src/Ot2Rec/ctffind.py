...
class ctffind:
    ...
    def __init__(
        ...
        if not os.path.isdir(self.params["System"]["output_path"]):
            subprocess.run(
                ["mkdir", self.params["System"]["output_path"]],
    ...
    def _get_images(self):
        ...
        for curr_ts in self._process_list:
            ...
            self.ctf_images = pd.concat([self.ctf_images, temp], ignore_index=True)
    ...
    def _set_output_path(self):
        ...
        self.ctf_images["file_paths"] = self.ctf_images.apply(
            lambda df: df["output"], axis=1
        )

        # update output column
        self.ctf_images["output"] = self.ctf_images.apply(
            lambda row: f"{self.params['System']['output_path']}/"
            f"{self.params['System']['output_prefix']}_{row['ts']}_{row['angles']}_ctffind.mrc",
            axis=1,
        )
    ...
    def _check_processed_images(self):
        ...
        if len(self.meta_out) > 0:
            self._missing = self.meta_out.loc[
                ~self.meta_out["output"].apply(lambda x: os.path.isfile(x))
            ]
        ...
        _ignored = self.ctf_images[
            self.ctf_images.output.isin(self.meta_out.file_paths)
        ]
        ...
        self._process_list = (
            self.ctf_images["ts"].sort_values(ascending=True).unique().tolist()
        )
    ...
    def _get_ctffind_command(self, image):
        ...
        input_dict = [
            ...
            str(self.params["ctffind"]["astigm_type"])
            if self.params["ctffind"]["astigm_type"]
            else "no",
    ...
    def _ctffind_single(self, idx):
        ...
        try:
            ...
            error_count += 1
            self.logObj(
                f"CTFFind4: An error has occurred ({ctffind_run.returncode}) "
                f"on stack{index}."
            )
    ...
    def run_ctffind(self):
        ...
        if error_count == 0:
            ...
            self.logObj(
                "WARNING: All Ot2Rec-CTTFFind4 jobs finished."
    ...
    def update_ctffind_metadata(self):
        ...
        _to_append = self.ctf_images.loc[
            self.ctf_images["output"].apply(lambda x: os.path.isfile(x))
        ]
        self.meta_out = pd.concat([self.meta_out, _to_append], ignore_index=True)
        self.ctf_images = self.ctf_images.loc[
            ~self.ctf_images["output"].apply(lambda x: os.path.isfile(x))
        ]
...
def update_yaml(args):
    ...
    if not os.path.isfile(ctf_yaml_name):
        logger(level="error", message="CTFFind4 config file not found.")
        raise IOError(
            "Error in Ot2Rec.main.update_ctffind_yaml: ctffind config file not found."
        )
    if not os.path.isfile(mc2_yaml_name):
        logger(level="error", message="MotionCor2 config file not found.")
        raise IOError(
            "Error in Ot2Rec.main.update_ctffind_yaml: motioncorr config file not found."
        )
    ...
    if os.path.isfile(ctf_md_name):
        ...
        with open(ctf_md_name, "r") as f:
            ctf_md = pd.DataFrame(yaml.load(f, Loader=yaml.FullLoader))[
                ["ts", "angles"]
            ]
    ...
    if is_old_project:
        merged_md = mc2_md.merge(ctf_md, how="outer", indicator=True)
    ...
    ctf_params = prmMod.read_yaml(
        project_name=args.project_name, filename=ctf_yaml_name
    )
    ...
    ctf_params.params["ctffind"]["pixel_size"] = mc2_params.params["MC2"][
        "desired_pixel_size"
    ]
...
def run(exclusive=True, args_in=None):
    ...
    if not os.path.isfile(ctffind_yaml):
        logger(level="error", message="CTFFind yaml config not found.")
        raise IOError(
            "Error in Ot2Rec.main.run_ctffind: ctffind yaml config not found."
        )
    if not os.path.isfile(mc2_md_file):
        logger(level="error", message="MC2 output metadata not found.")
        raise IOError(
            "Error in Ot2Rec.main.run_ctffind: MC2 output metadata not found."
        )

    # Read in config and metadata
    ctffind_config = prmMod.read_yaml(project_name=project_name, filename=ctffind_yaml)
    mc2_md = mdMod.read_md_yaml(
        project_name=project_name, job_type="ctffind", filename=mc2_md_file
    )
