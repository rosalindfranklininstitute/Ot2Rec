...
class RLF_deconv:
    ...
    def __call__(self):
        ...
        for idx in tqdm_iter:
            ...
            self.out_path = (
                f"{self.out_folder}/{self.plist[idx]}/{self.plist[idx]}_deconv.mrc"
            )
    ...
    def _get_internal_metadata(self):
        ...
        """
        # Get process list
        raw_subfolders = f"{self.raw_folder}/" f"{self.rootname}_*{self.suffix}"
        psf_subfolders = f"{self.kernel_folder}/" f"{self.rootname}_*{self.suffix}"
...
def run():
    ...
    logger(level="info", message="Ot2Rec-RLFDeconv started.")
