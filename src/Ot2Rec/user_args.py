...
def get_args_new_proj():
    ...
    parser.add_argument(
        ...
        default="",
        help=(
            "Common prefix of raw tilt series folder(s). "
    ...
    parser.add_argument(
        "--no_mdoc", action="store_true", help="Use this flag if no MDOC file provided."
    )
...
def get_args_mc2():
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument("project_name", type=str, help="Name of current project")
    parser.add_argument("pixel_size", type=float, help="Image pixel size in Angstroms.")
    parser.add_argument(
        "-o",
        "--output_folder",
        type=str,
    ...
    parser.add_argument(
        "--no_gpu", action="store_true", help="Use CPU only for motion-correction."
    )
    parser.add_argument(
        "-jpg",
        "--jobs_per_gpu",
    ...
    parser.add_argument(
        "-m", "--gpu_mem_usage", type=float, default=1, help="MotionCor2 memory usage."
    )
    parser.add_argument(
        "--exec_path",
        type=str,
        ...
        help=(
            "Path to MotionCor2 executable. "
            "(Default: /opt/lmod/modules/motioncor2/1.4.0/MotionCor2_1.4.0/MotionCor2_1.4.0_Cuda110)"
        ),
    )
    parser.add_argument(
        "--gain", type=str, help="Path to gain reference file. (Default: None)"
    )
    parser.add_argument(
        "--super_res",
        action="store_true",
    ...
    parser.add_argument(
        "--no_subgroups", action="store_true", help="Do not use subgroups in alignment."
    )
...
def get_args_ctffind():
    ...
    parser.add_argument(
        ...
        help=(
            "Min, max and step size of initial defocus search in Angstroms. "
            "(Default: 5000, 50000, 500)"
        ),
    )
    parser.add_argument(
...
def get_args_align():
    ...
    parser.add_argument(
        ...
        help=(
            "Path to template file of BatchRunTomo directives. "
            "(Default: /opt/lmod/modules/imod/4.11.1/IMOD/SystemTemplate/cryoSample.adoc)"
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=[2, 2],
        help=(
            "Patch-tracking: Maximum extent (in pixels) to which patches are allowed to "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fine-alignment: Do not apply weighting to entire contours from patch-tracking. "
            "Use flag if True."
        ),
    )
...
def get_args_align_ext():
    ...
    parser.add_argument(
        ...
        default=[2, 2],
        help=(
            "Patch-tracking: Maximum extent (in pixels) to which patches are allowed "
...
def get_args_savurecon():
    ...
    parser.add_argument(
        "-s", "--suffix", type=str, default="", help="Suffix of project files"
    )
    parser.add_argument(
        "-e",
        "--extension",
    ...
    parser.add_argument(
        "-is", "--imod_suffix", type=str, default="ali", help="IMOD file suffix"
    )
    parser.add_argument(
        "-o",
        "--output_path",
...
def get_args_rldeconv():
    ...
    """
    parser = argparse.ArgumentParser()
    parser.add_argument("image_path", type=str, help="Path to raw image.")
    parser.add_argument(
        "psf_path", type=str, help="Path to PSF for deconvolving raw image."
    )
    parser.add_argument(
        "--image_type",
        type=str,
    ...
    parser.add_argument(
        "output_path", type=str, help="Path to output (deconvolved) image."
    )
    parser.add_argument(
        "-d",
        "--device",
    ...
    parser.add_argument(
        "--uint", action="store_true", help="Store results as UInt8. Use flag if True."
    )
...
def get_args_aretomo():
    ...
    parser.add_argument("--pixel_size", type=float, help="Image pixel size in nm.")
    parser.add_argument(
        "-rn",
        "--rootname",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Binning to be applied to saved .mrc file." " Default = 4",
    )
    parser.add_argument(
        "--recon_algo",
        type=int,
        default=0,
        help="Reconstruction algorithm to be used." " 0: WBP (default)," " 1: SART",
    )
