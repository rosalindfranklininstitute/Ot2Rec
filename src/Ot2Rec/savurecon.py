...
class SavuRecon:
    ...
    def _get_internal_metadata(self):
        ...
        for curr_ts in self.params["System"]["process_list"]:
            subfolder = (
                f"{self.basis_folder}/{self.rootname}_{curr_ts:04d}{self.suffix}"
            )
    ...
    def _get_savuconfig_recon_command(self, i):
        ...
        ts_name = os.path.splitext(
            os.path.basename(self.params["Savu"]["setup"]["tilt_angles"][i])
        )[0]
        ...
        self.md_out["savu_process_lists"][curr_ts] = "{}/{}_{}.nxs".format(
            subfolder, ts_name, algo
        )
    ...
    def _create_savurecon_process_list(self, i):
        ...
        if savu_config.communicate()[1] is None:
            print(
                f"Process list created: {self.md_out['savu_process_lists'][curr_ts]}."
            )
...
def update_yaml(args):
    ...
    if not os.path.isfile(savu_yaml_name):
        logger(level="error", message="Savu metadata file not found.")
        raise IOError("Error in Ot2Rec.main.update_savu_yaml: File not found.")

    parent_path = args.stacks_folder.value
    rootname = (
        args.project_name.value if args.rootname.value == "" else args.rootname.value
    )
    ...
    # Find stack files
    st_file_list = glob(
        f"{parent_path}/{rootname}_*{suffix}/{rootname}*_{suffix}{imod_suffix}.{ext}"
    )
    st_file_list.sort()
    ...
    ts_list = [
        int(
            i.split("/")[-1]
            .replace(f"{rootname}_", "")
            .replace(f"_{suffix}{imod_suffix}.{ext}", "")
        )
    ...
    recon_params = prmMod.read_yaml(
        project_name=args.project_name.value, filename=savu_yaml_name
    )
    ...
    logger(
        message=f"Search term is {parent_path}/{rootname}_*{suffix}/{rootname}*_{suffix}{imod_suffix}.{ext}"
    )
...
def run():
    ...
    savurecon_params = prmMod.read_yaml(
        project_name=args.project_name, filename=savurecon_yaml
    )
