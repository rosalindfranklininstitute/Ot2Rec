...
@mg(
    ...
    excl_views={"widget_type": "LiteralEvalLineEdit", "label": "Excluded views"},
    output_folder={"label": "IMOD output folder", "mode": "d"},
    ...
    fiducial_size={
        "label": "Size of fiducial particles in nm (-1 if fiducial-free)",
        "min": -1.0,
        "step": 0.01,
    },
    num_beads={"label": "# of beads to track"},
    adoc_template={"label": "Path to BatchRunTomo directives template"},
    stack_bin_factor={"label": "Stack: Raw image stacks downsampling factor", "min": 1},
    delete_old_files={
        "label": "Preprocessing: Remove original stack when excluding views"
    },
    remove_xrays={"label": "Preprocessing: Remove X-rays and other artefacts"},
    coarse_align_bin_factor={
        "label": "Coarse-alignment: Coarse aligned stack binning",
        "min": 1,
    },
    num_patches={
        "widget_type": "LiteralEvalLineEdit",
    ...
    patch_overlap={
        "label": "Patch-tracking: % overlap between patches",
        "min": 0,
        "max": 100,
    },
    num_iter={
        "label": "Patch-tracking: Number of iterations (1-4)",
        "min": 1,
        "max": 4,
    },
    limits_on_shift={
    ...
    adjust_tilt_angles={
        "label": "Patch-tracking: Rerun patch-tracking with tilt-angle offset"
    },
    num_surfaces={
    ...
    weight_contours={
        "label": "Fine-alignment: Apply weighting to entire contours from patch-tracking"
    },
)
def get_args_align(
    project_name="",
    ...
    num_beads=0,
    adoc_template=Path(
        "/opt/lmod/modules/imod/4.11.1/IMOD/SystemTemplate/cryoSample.adoc"
    ),
    stack_bin_factor=4,
...
@mg(
    ...
    input_folder={"label": "Input folder with stacks", "mode": "d"},
    output_folder={"label": "IMOD output folder", "mode": "d"},
    file_prefix={"label": "File prefix (if different from project name)"},
    file_suffix={"label": "IMOD file suffix (if applicable)"},
    no_rawtlt={"label": "Ignore .rawtlt files?"},
...
@mg(
    ...
    bin_factor={"label": "Aligned stack: Binning factor for aligned stack", "min": 1},
    thickness={
        "label": "Reconstruction: Thickness (in pixels) for reconstruction *",
        "min": 0,
        "max": 50000,
        "step": 100,
    },
...
@mg(
    ...
    pixel_res={"label": "Pixel resolution (in angstroms) *", "min": 0.0, "step": 0.001},
    ds_factor={"label": "Alignment / reconstruction downsampling factor *", "min": 1},
...
@mg(
    ...
    niter={"label": "Max number of iterations used in deconvolution", "min": 1},
...
@mg(
    ...
    result_widget=False,
    project_name={"label": "Project name *"},
    stacks_folder={"label": "Path to parent folder with stacks", "mode": "d"},
    output_path={"label": "Path to output folder", "mode": "d"},
...
@mg(
    ...
    do_positioning={"label": "Positioning: Do positioning?"},
    bin_factor={"label": "Binning factor for stack", "min": 1},
    unbinned_thickness={
        "label": "Positioning: Unbinned thickness (in pixels) for samples or whole tomogram",
        "min": 0,
        "max": 50000,
...
def get_args_imod_route(
    mc2_path=Path(
        "/opt/lmod/modules/motioncor2/1.4.0/MotionCor2_1.4.0/MotionCor2_1.4.0_Cuda110"
    ),
    do_ctffind=False,
