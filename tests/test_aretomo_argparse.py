...
class AreTomoTest(unittest.TestCase):
    ...
    def _generate_yaml(self, working_dir, args, kwargs):
        ...
        aretomo_params = prmMod.read_yaml(
            args.project_name, self._get_aretomo_yaml_name(args)
        )
    ...
    def test_aretomo_align_defaults(self):
        ...
        args, kwargs = parser.parse_known_args(["TS", "0", "90"])

        aretomo_params = self._generate_yaml(self.temp_path, args, kwargs)

        self.assertEqual(aretomo_params.params["System"]["process_list"], [1])
        self.assertEqual(aretomo_params.params["AreTomo_setup"]["aretomo_mode"], 0)
        self.assertEqual(aretomo_params.params["AreTomo_setup"]["rot_angle"], 90)
        self.assertEqual(
    ...
    def test_aretomo_extra_kwargs(self):
        ...
        """
        parser = uaMod.get_args_aretomo()
        args, kwargs = parser.parse_known_args(["TS", "0", "90", "--extra", "kwargs"])
    ...
    def test_aretomo_recon_defaults(self):
        ...
        parser = uaMod.get_args_aretomo()
        args, kwargs = parser.parse_known_args(["TS", "1", "90", "--volz", "200"])

        aretomo_params = self._generate_yaml(self.temp_path, args, kwargs)

        self.assertEqual(aretomo_params.params["System"]["process_list"], [1])
        self.assertEqual(aretomo_params.params["AreTomo_setup"]["aretomo_mode"], 1)
        self.assertEqual(
            aretomo_params.params["AreTomo_setup"]["input_mrc"][0],
        ...
        self.assertTrue(aretomo_params.params["AreTomo_recon"]["volz"] > 0)
    ...
    def test_aretomo_alignrecon_defaults(self):
        ...
        parser = uaMod.get_args_aretomo()
        args, kwargs = parser.parse_known_args(["TS", "2", "90", "--volz", "200"])

        aretomo_params = self._generate_yaml(self.temp_path, args, kwargs)

        self.assertEqual(aretomo_params.params["System"]["process_list"], [1])
        self.assertEqual(aretomo_params.params["AreTomo_setup"]["aretomo_mode"], 2)
        self.assertEqual(
            aretomo_params.params["AreTomo_setup"]["input_mrc"][0],
            "./aretomo/TS_01/TS_01.st",
        )
