...
class AreTomoTest(unittest.TestCase):
    ...
    def _create_temp_files(self, ext, aretomo_mode, rootname="", suffix="", kv=[]):
        ...
        aretomo_params = prmMod.read_yaml(
            project_name="TS",
            filename=aretomo._get_yaml_filename(
                aretomo_mode,
                "TS"
            )
        )
    ...
    def test_aretomo_rootname_set(self):
        ...
        self.assertEqual(
            aretomo_params.params["System"]["output_rootname"],
            "test_rootname"
        )
        os.remove("./TS_aretomo_align.yaml")
    ...
    def test_aretomo_suffix_set(self):
        ...
        self.assertEqual(
            aretomo_params.params["System"]["output_suffix"],
            "test_suffix"
        )
        os.remove("./TS_aretomo_align.yaml")
    ...
    def test_aretomo_align_st_file_list(self):
        ...
        test_cases = [
            (
                {"rootname": "", "suffix": "", "aretomo_mode": 0},
                "TS_*/TS_*.st"
            ),
            (
            ...
            (
                {"rootname": "", "suffix": "", "aretomo_mode": 2},
                "TS_*/TS_*.st"
            ),
            (
        ...
        for rn_sf_dict, expected in test_cases:
            with self.subTest(f"{rn_sf_dict} -> {expected}"):
                ...
                self.assertEqual(
                    aretomo_params.params["AreTomo_setup"]["input_mrc"],
                    tmpsts
                )
                os.remove(aretomo._get_yaml_filename(
    ...
    def test_aretomo_output_mrc_set(self):
        ...
        for rn_sf_dict, search in test_cases:
            with self.subTest(f"{rn_sf_dict} -> {search}"):
                ...
                self.assertEqual(
                    aretomo_params.params["AreTomo_setup"]["output_mrc"],
                    expected
                )
                os.remove(aretomo._get_yaml_filename(
                    rn_sf_dict["aretomo_mode"], "TS"
