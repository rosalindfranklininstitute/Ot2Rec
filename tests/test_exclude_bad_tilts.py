...
class ExcludeBadTiltsSmokeTest(unittest.TestCase):
    ...
    def test_yaml_creation(self):
        ...
        self.assertTrue(os.path.isfile("./TS_exclude_bad_tilts.yaml"))

        params = prmMod.read_yaml(
            project_name="TS", filename="./TS_exclude_bad_tilts.yaml"
        )
    ...
    def test_EBT_metadata(self):
        ...
        with open(f"{tmpdir.name}/TS_exclude_bad_tilts_mdout.yaml", "r") as f:
            md = yaml.load(f, Loader=yaml.FullLoader)

        self.assertEqual(len(md["Excluded_Tilt_Index"][1]), 2)

        self.assertEqual(md["Excluded_St_Files"][1], "stacks/TS_0001/TS_0001.excl")

        self.assertEqual(list(md["Excluded_Tilt_Angles"][1]), [-60.0, 60.0])
    ...
    def test_EBT_recombine(self):
        ...
        exclude_bad_tilts._recombine_tilt_one_ts(0, params.params, md)
        ...
        np.testing.assert_allclose(ta, np.linspace(-60, 60, 10))
    ...
    def test_EBT_with_existing_dryrun_file(self):
        ...
        self.assertRaises(IOError, ebt_obj.run_exclude_bad_tilts, True)

        tilts_to_exclude = {1: [1, 2, 3, 9]}
